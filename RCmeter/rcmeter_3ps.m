function ret = rcmeter_3p()
  ...
  for epoch=1:1:EpochMax
    ...
    for row= 1:1:rows
      ROW=[];
      n_r=row; #((row-1)*batchSize)+1:1:(row*batchSize);
      for p=1:1:a1th
  ...
  title('Current (A)');
  subplot(4,2,3);
  plot(n_t, G1_t, 'b-', n_t, G1_r, 'rx');
  title('Conductance (S)');
  subplot(4,2,5);
  plot(n_t, G2_t, 'b-', n_t, G2_r, 'rx');
  title('Conductance (S)');
  subplot(4,2,7);
  plot(n_t, G3_t, 'b-', n_t, G3_r, 'rx');
  title('Conductance (S)');
  subplot(4,2,4);
  plot(n_t, C1_t, 'b-', n_t, C1_r, 'rx');
  title('Capacitance (F)');
  subplot(4,2,6);
  plot(n_t, C2_t, 'b-', n_t, C2_r, 'rx');
  title('Capacitance (F)');
  subplot(4,2,8);
  plot(n_t, C3_t, 'b-', n_t, C3_r, 'rx');
  title('Capacitance (F)');
